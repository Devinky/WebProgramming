...
public class ajaxDao {
	...
	public ArrayList<ajaxMember> updateView(String userName) {
		ArrayList<ajaxMember> list = new ArrayList<>();;
		...
		try {
			conn = DBConnect.getConnection();
			query = "select * from ajax1 where userName=?";
			pstm = conn.prepareStatement(query);
			...
			if (rs != null) {
				while (rs.next()) {
					...
					list.add(new ajaxMember(userName2, userAge, userGender, userAddr));
				}
			}
		} catch (SQLException e) {
			e.printStackTrace();
